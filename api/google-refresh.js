// Serverless function to refresh Google Calendar tokens using stored cookie
module.exports = async function handler(req, res) {
  ...
  let tokens
  try {
    tokens = JSON.parse(Buffer.from(match[1], 'base64').toString())
  } catch {
    return res.status(401).json({ error: 'Invalid token cookie' })
  }

  // Return access token if still valid
  ...
  return res.status(200).json({ access_token: fresh.access_token, expires_at: newTokens.expires_at })
}
